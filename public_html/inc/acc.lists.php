<?php 
	...
	if($userdata->ownership == 0){
		...
		if(isset($_GET['list'])){
			if($_GET['list'] == 'all'){
				$query = mysql_query("SELECT * FROM `saved_builds` WHERE `owner`='{$token}' ORDER BY `id` DESC LIMIT {$offset}, {$offMod}");
				$count = mysql_num_rows(mysql_query("SELECT * FROM `saved_builds` WHERE `owner`='{$token}' ORDER BY `id` DESC"));
				$listData = (object) array('listtitle'=>'All Builds','listid'=>$user);
			} else {
				$query = mysql_query("SELECT * FROM `saved_builds` WHERE `owner`='{$token}' AND `listid`='{$_GET['list']}' ORDER BY `id` DESC LIMIT {$offset}, {$offMod}");
				$count = mysql_num_rows(mysql_query("SELECT * FROM `saved_builds` WHERE `owner`='{$token}' AND `listid`='{$_GET['list']}' ORDER BY `id` DESC"));
				$listData = mysql_fetch_object(mysql_query("SELECT * FROM `lists` WHERE `owner`='{$token}' AND `listid`='{$_GET['list']}'"));
				if(empty($listData->listtitle)){
					$listData = (object) array('listtitle'=>'All Builds','listid'=>$user);
				}
			}
		} else {
			...
			$query = mysql_query("SELECT * FROM `saved_builds` WHERE `owner`='{$token}' AND `listid`='{$listData->listid}' ORDER BY `id` DESC LIMIT {$offset}, {$offMod}");
			$count = mysql_num_rows(mysql_query("SELECT * FROM `saved_builds` WHERE `owner`='{$token}' AND `listid`='{$listData->listid}' ORDER BY `id` DESC"));
		}
		...
		<div class="buildList2 expadd" style="margin-bottom:15px;">
			...
			<div><input type="text" style="width:100%;" id="slinks" value="mugenmonkey.com/darksouls/share?<?php if(isset($_GET['list']) && $_GET['list'] == 'all' || !isset($_GET['list'])){echo 'uid='. $listData->listid;}else {echo 'lid='. $listData->listid;} ?>"/></div>
		</div>
		...
		if(mysql_num_rows($query) == 0){
			...
			while($data = mysql_fetch_array($query)){
				echo '<div class="buildrow clearfix"><div class="buildtitle buildrow2 full"><input type="checkbox" value="'.$data['ref'].'" /> <a href="/darksouls/?c='.$data['ref'].'">'.$data['title'].'</a><span class="faded small toright">';
