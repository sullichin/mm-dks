...
var oldURL='',aaa,aab,aac,aad,aae=[],aaf=[],aag=[],aah=[],aai=[],g=false,eff=[],aaj=aak=aal=aam=aan=aaq=aap=aar=aas=aat=aau=defp=aav=aax=aay=aaz=cah=gre=0,bba,bbb,bbc,bbd,bbf,ring1,ring2,gpth=true,rings = [],armCache = [],tbr=clr=have=rosp=sspr=fspr=tspr=sppr=bbr=pbr=cbr=rtsr=btsr=bdr=ldr=sdr=dcr=wsr=dsr=rotsf=lr=wr=hr=hor=bdsag=dwgr=rir=cgsr=cssr=ocr=rof=rofap=rotee=ewgr=rotsp=avarice=dusk=gata=blds=dbf=drak=dgs=dkg=dgt=mgs=false,mof=mom=fthr=false,dksdef=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]],dksalpha=[],oldStat={stats:[],weapons:[],armor:[],rings:[],h2:''},nerf=true,specmod=1,armorWeight,stampen=gras=chil=0;
$(document).ready(function(){
	...
	function applyEn(){
		...
		if(oldStat.stats[2] != index || 
			...
			bbc = zz.stam[index];
			xy.eqp.val(aan + '/' + (aaq(zz.bur[index])));
			var total = aaq(zz.bur[index]);
			var f = roundNumber(total/aan,4),g = roundNumber((aan/total)*100,4).toFixed(1);
			var c,text,untilf,untilh;
			var fourth = roundNumber(total/4,4);
			var half = roundNumber(total/2,4);
			
			
			if(f>=4 || dwgr && f>=2){
				c = 'green';
				text = 'Fast Roll';
				untilf = roundNumber(fourth - aan,1).toFixed(1);
				if(dwgr){
					untilf = roundNumber(half - aan,1).toFixed(1);
					text = 'Ninja Flip';
				}
			}else if(f>2){
				c = 'blue';
				...
				c = 'red';
				text = 'Fat Roll';
				untilf = roundNumber(total-aan,1).toFixed(1);
			}else if(f>0){
				c = 'deepred';
				text = 'Slow Walk';
				untilf = roundNumber(total - aan,1).toFixed(1);
			...
			if(bdsag)
			l = 1.5 * specmod;
			xy.ble.val(Math.floor(Math.round((zz.ble[index]) * (y * l * g * t) + aap)));
			bbf = zz.ble[index];
		} else {
			return false;
		}
	...
	function getArmorStats(){
		...
		var hob,aob,gob,kob;
		hob = head[id1]
			hob = (id1 == '4' && !nerf) ? zz.altEk.head : head[id1];
			aob = (id2 == '4' && !nerf) ? zz.altEk.chest : aaf[id2];
			gob = (id3 == '4' && !nerf) ? zz.altEk.hands : aag[id3];
			kob = (id4 == '4' && !nerf) ? zz.altEk.legs : aah[id4];
	...
	function calcAR(pas,se){
		if(oldStat.stats[3] != xy.st.val() || 
			...
			if(mods) {
				...
				if((typ == 'normal' || typ == 'crystal' || typ== 'raw') && weptyp != 'u'){
					...
					if(reg[0] > strmuf || reg[1] > parseInt(xy.dx.val())) {
						...
						phya = Math.floor(mod[0]*(1+(mod[6]*str)+(mod[7]*dex)));
						mag =  0;
						fir = 0;
						lgh = 0;
						if(weptyp == 'b' || weptyp == 'cb'){
							phya += adds[0];
							mag += adds[1]
							fir += adds[2];
							lgh += adds[3];
				...
				if((typ == 'magic' || typ == 'enchanted' || typ== 'devine' || typ== 'occult') && (weptyp != 'u')){
					...
					if(reg[0] > strmuf || reg[1] > parseInt(xy.dx.val()) || reg[2] > parseInt(xy.it.val()) || reg[3] > parseInt(xy.ft.val())) {
						...
						phya = Math.floor(mod[0]*(1+(mod[6]*str)+(mod[7]*dex)));
						mag =  Math.floor(mod[1]*(1+(modi*ma)));
						fir = mod[2];
						lgh = mod[3];
						if(weptyp == 'b' || weptyp == 'cb'){
							phya += adds[0];
				...
				if(typ == 'chaos' && weptyp != 'u'){
					...
					if(reg[0] > strmuf || reg[1] > parseInt(xy.dx.val())) {
						...
						phya = Math.round(mod[0]*(1+(.21*cha)));
						mag = 0;
						fir = Math.round(mod[2]*(1+(0.2075*cha)));
						lgh = 0;
						if(weptyp == 'b' || weptyp == 'cb'){
							phya += adds[0];
							mag += adds[1]
				...
				if(weptyp == 'u'){
					...
					if(reg[0] > strmuf || reg[1] > parseInt(xy.dx.val()) || reg[2] > parseInt(xy.it.val()) || reg[3] > parseInt(xy.ft.val())) {
						...
						phya = Math.floor(mod[0]*(1+(mod[8]*str)+(mod[9]*dex)));
						fir = mod[2];
						lgh = mod[3];
						if(fal == 'ChaosBlade' || fal == 'QuelaagsFurysword'){
							phya = Math.round(phya*(1+(.215*cha)))
							fir = Math.round(fir*(1+(0.20658*cha)))
						}
						mag =  Math.floor(mod[1]*(1+(mod[10]*itt)+(mod[11]*fat)));
			...
			if(weptyp == 's'){
				...
				var dat = zz.modifiers.normal.s[index];
			...
			if(weptyp == 'b' || weptyp == 'cb' || fal == 'DragonslayerGreatbow' || fal == 'DarkmoonBow'){
	...
	function getArrowDamage(TYPE,WEPTYPE,HAND,WEAPON){
		...
		if(TYPE == 'dragon' && WEAPON != 'DragonslayerGreatbow'){
			if(WEPTYPE == 'b'){
				a = wood;
				HAND.attr('typ','wooden');
			} else {
	...
	function makeTypeModal(pos,hen,sel,typ,OBJECT) {
			...
			var arrows = ['wooden','standard','large','feather','firea','poison','moonlight','dragon'];
			...
			if(ind == 0)
				...
				showAll();
				wep = ind - 104;
			}else if(ind > 107 && ind <= 111 && TYPE){
				if($('.wepTyp[tpy="'+typ+'"]').css('display') == 'none'){
					$('#tynor').addClass('activ');
				}
				hideCross();
				b = 115; //////////////////////////////////////////////////////////////////
			} else if(ind == 102 || ind > 103 && ind <= 107){ //start arrow selector
	...
	function showArrows(wep){
		...
		$('#tybwood,#tybstand,#tybheavy,#tybsniper,#tyblight,#tydrag').css('display','none');
	...
	function showBolts(){
		...
		$('#tywood,#tystand,#tylarge,#tyfeather,#tyfire,#typoison,#tymoon,#tydrag').css('display','none');
		$('#wepTypeSel').css('height','96px')
		$('#arpo .wepTyp').each(function(){
			$(this).removeClass('activ');
		});
		...
		if(oldStat.stats[6] != xy.it.val() || 
			...
			obj.each(function(){
				if($(this).css('display') != 'none'){
					...
					if(too == 'Magic' || too == 'Miracles'){
						var typ = (too == 'Magic') ? parseInt(xy.it.val()) : parseInt(xy.ft.val());
	...
	function aaq(value){ 
		...
		if(have)
		 g = 1.5;
		return roundNumber(parseInt(value)*(p*e*g),1).toFixed(1);
	}
	function aat(value){
		var p = 1;
		if(rofap)
...
*/
	...
	if(a/b<1)
		c = 0.98 * ( 0.4*(Math.pow(a,3)/Math.pow(b,2))-0.09*(Math.pow(a,2)/b)+0.1*a)
	else
		c = 0.98 * ( a-(b*(0.79 * Math.exp(-0.27*(b/a)))))
