<?php
	...
	if(mysql_num_rows($dit) == 0){
		$nref = (int) round(mt_rand(100000000,400000000) * 1.23653) . mt_rand(1000,9000) . mt_rand(600,900);
		if(isset($_POST['til']) && isset($_POST['aut'])) {
			...
			$phy = filter_var($_POST['phy'],FILTER_VALIDATE_INT);
			$strike = filter_var($_POST['strike'],FILTER_VALIDATE_INT);
			$slash = filter_var($_POST['slash'],FILTER_VALIDATE_INT);
			$thrust = filter_var($_POST['thrust'],FILTER_VALIDATE_INT);
			$magic = filter_var($_POST['magic'],FILTER_VALIDATE_INT);
			$flame = filter_var($_POST['flame'],FILTER_VALIDATE_INT);
			$light = filter_var($_POST['light'],FILTER_VALIDATE_INT);
			$poise = filter_var($_POST['poise'],FILTER_VALIDATE_INT);
			$bleed = filter_var($_POST['bleed'],FILTER_VALIDATE_INT);
			$poison = filter_var($_POST['poison'],FILTER_VALIDATE_INT);
			$curse = filter_var($_POST['curse'],FILTER_VALIDATE_INT);
			$desc = $_POST['desc'];
			...
			$bvalues = "'{$soul}','{$vit}','{$att}','{$end}','{$str}','{$dex}','{$res}','{$int}','{$fai}','{$hp}','{$stam}','{$phy}','{$strike}','{$slash}','{$thrust}','{$magic}','{$flame}','{$light}','{$poise}','{$bleed}','{$poison}','{$curse}','{$ndesc}'";
			
			
			
			$public = filter_var($_POST['public'],FILTER_VALIDATE_INT);
			$public = (int) $public;
			...
			if(strlen($title) < 4 || strlen($author) < 4){
				...
				$match = mysql_query("SELECT `id` FROM `saved_builds` WHERE `b64`='{$base}'");
				if($author != 'Anonymous'){
				...
				if(mysql_num_rows($match) > 0){
					...
					echo json_encode(array('a'=>$u->ref,"b"=>'Build already made'));
				} else {
					...
					if(isset($_COOKIE['user']) && isset($_COOKIE['token'])){
						if($newlist){
							...
							$kaj = mysql_query("INSERT INTO `lists` (owner,listid,listtitle,defaulti,datei,alias) VALUES('{$hashtag}','{$newListID}','{$newlist}','0',NOW(),'{$author}')") or die(mysql_error());
						} else {
