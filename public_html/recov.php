		...
		if(empty($error)){
			$equery = mysql_query("SELECT * FROM `users` WHERE `email`='{$email}'");
				...
				if($_emailQuery == 1){
					...
					if($reco) {
						$to = $data->email;
						$subject = "Mugenmonkey Recovery Help";
						$messagem = "Follow this <a href='http://mugenmonkey.com/darksouls/reset?a=$random_genlink&h=$userhash'>link</a> to reset your password <br><br><br> If you did not request this email and believe someone is trying to access your account, please report it by emailing me <a href='mailto:report@mugenmonkey.com'>here</a>. <br> Otherwise, please ignore this message.";
						...
						if(!mail($to,$subject,$messagem,$headers)) {
							...
							$message = "Mail Sent. Please make your spam box.";
						}
					} else {
...
<?php include('thead.php'); ?>
  ...
  <script>window.jQuery || document.write('<script src="js/libs/jquery-1.7.1.min.js"><\/script>')</script>
	<script>$(document).ready(function(){$('.login').click(function(){$(this).addClass('hover');$('.login_form').addClass('shown');if(!$('.login').hasClass('intent')){$('.login').addClass('intent');$('.login,.login_form').click(function(event){event.stopPropagation();});} else if($('.login').hasClass('intent')){$('.login').removeClass('hover intent');$('.login_form').removeClass('shown');$(document).unbind('click');}$(document).click(function(){$('.login').removeClass('hover intent');$('.login_form').removeClass('shown');$(document).unbind('click');});return false;});});</script>
</body>
</html>